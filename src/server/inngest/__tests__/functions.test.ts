...
describe("Inngest Functions - coreAnalysisLogic", () => {
    ...
    it("should perform shallow clone and read from local FS during dependency analysis", async () => {
        ...
        (getRepoCommits as Mock<any>).mockResolvedValue([{ sha: "sha-commit", commit: { message: "init", author: { name: "test" } } }]);
        ...
        expect(gitMock.clone).toHaveBeenCalledWith(testData.githubUrl, expect.stringContaining(".tmp/repo-123"), expect.arrayContaining(["--depth", "1"]));
        
        // Verify local file read was attempted
        expect(fs.readFile).toHaveBeenCalledWith(expect.stringContaining("src/index.ts"), "utf8");
        
        // Verify cleanup was called
        expect(fs.rm).toHaveBeenCalledWith(expect.stringContaining(".tmp/repo-123"), expect.objectContaining({ recursive: true }));
    });

    it("should fallback to API if local file read fails", async () => {
        const { getLatestAnalysis } = await import("../../dal/analysis");
        const { getRepoTree, getFileContentFromRaw } = await import("../../octokit");
        const fs = (await import("node:fs/promises")).default;
        ...
        (fs.readFile as Mock<any>).mockRejectedValueOnce(new Error("File not found"));
        ...
        expect(getFileContentFromRaw).toHaveBeenCalledWith(expect.objectContaining({
            path: "src/index.ts"
        }));
    });
});
